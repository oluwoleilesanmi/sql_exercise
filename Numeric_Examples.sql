...
-- how the subject and total number of students who A_STRONGLY_AGREE to question 22 for each of the subjects '(8) Computer Science' and '(H) Creative Arts and Design'.
...
SELECT institution, ROUND(AVG(score*response)/AVG(response)) AS score FROM nss
...
SELECT institution, SUM(sample) AS sample,
       (SELECT sample
          FROM nss
         WHERE question = 'Q01'
           AND institution = n1.institution
